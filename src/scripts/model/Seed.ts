...
export class Seed {
	...
	static randomString() {
		...
		for (let i = 0; i < SEED_LENGTH; i++) {
			randomSeed += SEED_CHARS.charAt(
				Math.floor(Math.random() * SEED_CHARS.length),
			);
		}
		return randomSeed;
	}
	...
	static dailyString() {
		...
		let string =
			isoString.substring(0, 3) +
			isoString.substring(5, 6) +
			isoString.substring(7, 8);
	...
	static seededRandomString(rng: { random: () => number }) {
		...
		for (let i = 0; i < SEED_LENGTH; i++) {
			randomSeed += SEED_CHARS.charAt(
				Math.floor(rng.random() * SEED_CHARS.length),
			);
		}
