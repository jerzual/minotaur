		...
		this.level.rooms.forEach((room: Room) => {
			room.doors.forEach((door: Door) => {
				...
				if (freeDoors.length) {
					...
					console.log(
						`path from ${door.x},${door.y} to ${nextDoor.x},${nextDoor.y}`,
					);
					try {
						//Reset grid
